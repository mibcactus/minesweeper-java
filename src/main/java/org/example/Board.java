...
public class Board {
    ...
    public void printBoard(){
        ...
        for (int i = 0; i < board.size(); i++) {
            str = new StringBuilder(String.format("%2d: ", i));
            for (int j = 0; j < board.get(i).size(); j++) {
                str.append(String.format("%b  ", board.get(i).get(j).hasMine()));
            }
    ...
    public void generateBoard(){
        ...
        for(int i = 0; i < _properties._width; i++){
            ArrayList<Tile> row = new ArrayList<>();
            for(int j = 0; j < _properties._height; j++){
